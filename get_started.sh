...
DEFINE_string 'auv' 'AUV8' 'The auv targeted' 'a'
...
clean_workflows() {
    if [ $FLAGS_gpu == true ]; then
        if [ $FLAGS_type == "perception" ]; then
            ...
            rm .github/workflows/docker-image-perception-fix.yml
...
insert_name() {
    ...
    sed -i.bak "s/<ENTER_THE_AUV_TARGET>/${FLAGS_auv}/g" ".devcontainer/devcontainer.json" && rm .devcontainer/devcontainer.json.bak
